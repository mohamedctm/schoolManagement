  ...
  return (
    <div className="p-6 max-w-lg mx-auto h-screen">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href="/employees" className="bg-white text-black px-4 py-2 rounded hover:bg-red-600 hover:text-white">Back to Employees</Link>
      </div>
      ...
      <div className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <input type="text" placeholder="Username" className="w-full p-2 border border-gray-300 rounded mb-2" value={employee.username || ""} onChange={(e) => setEmployee({ ...employee, username: e.target.value })} required />
          <input type="password" placeholder="Password" className="w-full p-2 border border-gray-300 rounded mb-2" value={employee.password || ""} onChange={(e) => setEmployee({ ...employee, password: e.target.value })} required />
        </div>
        <button onClick={handleUpdate} className="w-full bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Update Credentials</button>
      </div>
    </div>
  );
}
